...
for i in range(dataCountTrain):
	XTrainImage = Image.fromarray(x_train[i])
	new_image = XTrainImage.resize((32,32), Image.HAMMING)
...
## Looping over the entire test set to rotate all images by specific degrees
xTestTempRotate = []
...
for degree in degreesList:
    ...
    for i in range(dataCountTest):
        ...
        xTestTempRotate.append(np.array(img_array_rot))
    x_test_rotated = np.array(xTestTempRotate)

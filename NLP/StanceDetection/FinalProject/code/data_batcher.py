...
class Batch(object):
    ...
    def __init__(self, context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, uuids=None):
        ...
        self.batch_size = len(self.context_ids) # in place of context_tokens

class CustomBatch(object):
    """A class to hold the information needed for a training batch"""

    def __init__(self, body_ids, body_mask, body_tokens, headline_ids, headline_mask, headline_tokens, ans_span, uuids=None):
        """
        Inputs:
          {context/qn}_ids: Numpy arrays.
            Shape (batch_size, {context_len/question_len}). Contains padding.
        ...
        self.headline_ids = headline_ids
        self.headline_mask = headline_mask
        ...
        self.batch_size = (self.body_ids).size
...
def custom_refill_batches(batches, word2id, body_file, headline_file, ans_file, batch_size, context_len, question_len, discard_long):
    """
    Adds more batches into the "batches" list.
    ...
    tic = time.time()
    examples = [] # list of (qn_ids, context_ids, ans_span, ans_tokens) triples
    body_line, headline_line, ans_line = body_file.readline(), headline_file.readline(), ans_file.readline() # read the next line from each

    while body_line and headline_line and ans_line: # while you haven't reached the end

        # Convert tokens to word ids
        body_tokens, body_ids = sentence_to_token_ids(body_line, word2id)
        headline_tokens, headline_ids = sentence_to_token_ids(headline_line, word2id)
        ans_span = intstr_to_intlist(ans_line) # Listof single number

        # read the next line from each file
        body_line, headline_line, ans_line = body_file.readline(), headline_file.readline(), ans_file.readline()

        # get ans_tokens from ans_span
        assert len(ans_span) == 1
# =============================================================================
#         if ans_span[1] < ans_span[0]:
#             print "Found an ill-formed gold span: start=%i end=%i" % (ans_span[0], ans_span[1])
#             continue
...
# =============================================================================
        ...
        # discard or truncate too-long questions
        if len(headline_ids) > question_len:
            if discard_long:
                continue
            else: # truncate
                headline_ids = headline_ids[:question_len]

        # discard or truncate too-long contexts
        if len(body_ids) > context_len:
            if discard_long:
                continue
            else: # truncate
                body_ids = body_ids[:context_len]

        # add to examples
        examples.append((body_ids, body_tokens, headline_ids, headline_tokens, ans_span))
    ...
    for batch_start in xrange(0, len(examples), batch_size):
        ...
        # Note: each of these is a list length batch_size of lists of ints (except on last iter when it might be less than batch_size)
        body_ids_batch, body_tokens_batch, headline_ids_batch, headline_tokens_batch, ans_span_batch = zip(*examples[batch_start:batch_start+batch_size])

        batches.append((body_ids_batch, body_tokens_batch, headline_ids_batch, headline_tokens_batch, ans_span_batch))
...
def custom_get_batch_generator(word2id, body_path, headline_path, ans_path, batch_size, context_len, question_len, discard_long):
    """
    This function returns a generator object that yields batches.
    The last batch in the dataset will be a partial batch.
    Read this to understand generators and the yield keyword in Python: https://stackoverflow.com/questions/231767/what-does-the-yield-keyword-do
    ...
    while True:
        if len(batches) == 0: # add more batches
            custom_refill_batches(batches, word2id, body_file, headline_file, ans_file, batch_size, context_len, question_len, discard_long)
        if len(batches) == 0:
            break

        # Get next batch. These are all lists length batch_size
        (body_ids, body_tokens, headline_ids, headline_tokens, ans_span) = batches.pop(0)
        ...
        # Make into a Batch object
        batch = CustomBatch(body_ids, body_mask, body_tokens, headline_ids, headline_mask, headline_tokens, ans_span)

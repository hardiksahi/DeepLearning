...
from data_batcher import padded, Batch, CustomBatch
from stance_preprocess import clean, get_tokenized_sequences
...
def custom_refill_batches(batches, word2id, body_token_data, headline_token_data, batch_size, context_len, question_len):
    """
    ...
    # Get next example
    body_tokens, headline_tokens = readnext(body_token_data), readnext(headline_token_data)

    while body_tokens and headline_tokens:

        # Convert context_tokens and qn_tokens to context_ids and qn_ids
        body_ids = [word2id.get(w, UNK_ID) for w in body_tokens]
        headline_ids = [word2id.get(w, UNK_ID) for w in headline_tokens]
        ...
        examples.append((body_ids, headline_ids))
    ...
    for batch_start in xrange(0, len(examples), batch_size):
        body_ids_batch, headline_ids_batch = zip(*examples[batch_start:batch_start + batch_size])

        batches.append((body_ids_batch, headline_ids_batch))
...
def get_preprocessed_data(dataset, stance_list, tier):
    
    body_token_data_list = []
    headline_token_data_list = []
    body_id_list, headline_list = [], []
    ...
    for stance in tqdm(stance_list, desc="Preprocessing {}".format(tier)):
        ...
        body_token_data_list.append(b_tokens)
        headline_token_data_list.append(h_tokens)
        body_id_list.append(stance['Body ID'])
    
    return body_token_data_list, headline_token_data_list, body_id_list, headline_list

#word2id, body_token_data_list, headline_token_data_list, model.FLAGS.batch_size, model.FLAGS.context_len, model.FLAGS.question_len    


def get_answers(sess, model, word2id, body_token_data_list, headline_token_data_list):
    data_size = len(body_token_data_list)
    num_batches = ((data_size-1) / model.FLAGS.batch_size) + 1
    batch_num = 0
    
    return_pred_answer_list = []
    
    print("Generating answers...")
    
    for batch in custom_get_batch_generator(word2id, body_token_data_list, headline_token_data_list, model.FLAGS.batch_size, model.FLAGS.context_len, model.FLAGS.question_len):
        
        pred_class = model.get_class(sess, batch)
        pred_class_list = pred_class.tolist()
        pred_class_label_list = [LABELS[int(a)] for a in pred_class_list]
        
        return_pred_answer_list = return_pred_answer_list+pred_class_label_list
    ...
    return return_pred_answer_list
